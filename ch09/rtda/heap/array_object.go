...
func ArrayCopy(src, dst *Object, srcPos, dstPos, length int32){
	...
	case []int16:
		...
		_dst := src.data.([]int16)[dstPos: dstPos+length]
		copy(_dst,_src)
	case []int32:
		_src := src.data.([]int32)[srcPos: srcPos+length]
		_dst := src.data.([]int32)[dstPos: dstPos+length]
		copy(_dst,_src)
	case []int64:
		_src := src.data.([]int64)[srcPos: srcPos+length]
		_dst := src.data.([]int64)[dstPos: dstPos+length]
		copy(_dst,_src)
	case []float32:
		_src := src.data.([]float32)[srcPos: srcPos+length]
		_dst := src.data.([]float32)[dstPos: dstPos+length]
		copy(_dst,_src)
	case []float64:
		_src := src.data.([]float64)[srcPos: srcPos+length]
		_dst := src.data.([]float64)[dstPos: dstPos+length]
		copy(_dst,_src)
	case []uint16:
		_src := src.data.([]uint16)[srcPos: srcPos+length]
		_dst := src.data.([]uint16)[dstPos: dstPos+length]
		copy(_dst,_src)
	case []*Object:
